...
// Paints one note for each drawing motion, color depends on the frequency 

//todo: fix x, y z positions
// play only one note at a time
// set only one color at a time
// change xpos and ypos and notes to be internal parameters? 
// make different color picker with different frequencies 
// add a clear screen button, also change screen to white background
// try recording works and see if it plays
...
class Paint : public SynthVoice {
 public:
  ...
  void onProcess(Graphics& g) override {
    ...
    // Now draw
    g.pushMatrix();
    //std::cout<< (x_pos/200.0)-2<< " , " << (y_pos/-400.0)<<std::endl;  
    //g.translate(frequency / 200 - 3, amplitude, -8);
    //std::cout<<frequency << " , " <<amplitude<<std::endl;  
    g.translate((x_pos/200.0)-1, (y_pos/-400.0), -10);
 ...
 public:
    ...
    if (k.shift()) {
      ...
      while(notes.size() > 0) {
          for (int i = 0; i < notes.size();i++) {
            synthManager.triggerOff(notes[i]);
            std::cout<< "clearing in keypress " <<notes[i] <<std::endl;
          }
      }
    }
      else {
  ...
  bool onMouseUp(Mouse const & m) override{
    ...
    for (int i = 0; i < notes.size();i++){
      if (notes[i] > 0) {
          synthManager.voice()->setInternalParameterValue(
              "frequency", ::pow(2.f, (notes[i] - 69.f) / 12.f) * A4);
          synthManager.triggerOn(notes[i]);
        }
    ...
    for (int i = 0; i < notes.size();i++){
        synthManager.triggerOff(notes[i]);
        std::cout<< "clearing " <<notes[i] <<std::endl;
    }
